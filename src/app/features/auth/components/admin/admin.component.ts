      ...
      <div class="mb-8">
        <nav class="flex space-x-4">
          <button
            ...
            [class.bg-blue-600]="isUsers()"
            [class.text-white]="isUsers()"
            [class.text-gray-700]="!isUsers()"
          ...
          <button
            ...
            [class.bg-blue-600]="isTickets()"
            [class.text-white]="isTickets()"
            [class.text-gray-700]="!isTickets()"
            class="px-4 py-2 rounded-md font-medium hover:bg-blue-700 hover:text-white transition-colors"
          >
            Tickets
          </button>
...
export class AdminComponent implements OnInit {
  ...
  // BONUS: derived signals pour simplifier le template
  isUsers = computed(() => this.activeTab() === 'users');
  isTickets = computed(() => this.activeTab() === 'tickets');
  ...
  ngOnInit(): void {
    ...
    const currentUser = this.authService.getCurrentUser(); // synchro dans ton service
    if (!currentUser || currentUser.role !== 'admin') {
      this.router.navigate(['/todos']);
      return;
    }

    // Charger les données (promesses dans tes services mock)
    void this.loadUsers();
    void this.loadTodos();
  }
  ...
  async loadUsers(): Promise<void> {
    try {
      const list = await this.authService.getAllUsers(); // Promise<User[]>
      this.users.set(list);
    } catch (error) {
  ...
  async loadTodos(): Promise<void> {
    try {
      const list = await this.todoService.getAllTodos(); // Promise<Todo[]>
      this.todos.set(list);
    } catch (error) {
