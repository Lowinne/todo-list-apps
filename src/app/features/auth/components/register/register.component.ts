...
export class RegisterComponent {
  ...
  onSubmit(): void {
    ...
    const body: RegisterRequest = this.registerForm.getRawValue() as RegisterRequest;

    from(this.authService.register(body)).subscribe({
      next: ({ success, user, error }: AuthResult) => {
        this.loading.set(false);
        if (success && user) {
          this.router.navigate(['/todos']);
        } else {
          this.error.set(error ?? 'Erreur lors de la création du compte');
        }
      },
      error: (err: unknown) => {
        this.loading.set(false);
